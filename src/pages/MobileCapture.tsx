...
// Auto-detect API URL: use current hostname when accessed via ngrok or network
// This function is called at runtime to get the current API URL
// Priority: ngrok detection > env variable > auto-detection > localhost
const getApiBaseUrl = () => {
  // Get current location info at runtime FIRST
  const hostname = window.location.hostname
  const protocol = window.location.protocol
  const port = window.location.port
  const fullUrl = window.location.href
  
  console.log('[MobileCapture] Location detection:', { hostname, protocol, port, fullUrl })
  
  // PRIORITY 1: Check for ngrok in hostname (various ngrok domain patterns)
  // If accessing via ngrok, ALWAYS use ngrok URL (ignore env variable)
  if (hostname.includes('ngrok') || hostname.includes('ngrok-free') || hostname.includes('ngrok.io') || hostname.includes('ngrok.app')) {
    // For ngrok, API is on the same domain (Flask serves both)
    // Don't add port for ngrok as it uses standard ports
    const baseUrl = `${protocol}//${hostname}`
    console.log('[MobileCapture] ✓ Detected ngrok domain, using API URL:', baseUrl, '(env variable ignored)')
    return baseUrl
  }
  
  // PRIORITY 2: Check env variable (only if not on ngrok)
  const envUrl = import.meta.env.VITE_API_BASE_URL
  if (envUrl) {
    console.log('[MobileCapture] Using env API_BASE_URL:', envUrl)
    return envUrl
  }
  
  // PRIORITY 3: Auto-detect based on hostname
  // If not localhost, assume it's a network access or tunnel
  if (hostname !== 'localhost' && hostname !== '127.0.0.1') {
    // Check if it's a local network IP
    if (hostname.match(/^192\.168\.|^10\.|^172\.(1[6-9]|2[0-9]|3[01])\./)) {
      // Local network IP - try port 5000
      const baseUrl = `${protocol}//${hostname}:5000`
      console.log('[MobileCapture] ✓ Detected local network IP, API URL:', baseUrl)
      return baseUrl
    } else {
      // Other domain (could be tunnel service or custom domain)
      // Use same domain, Flask serves both frontend and API
      const baseUrl = `${protocol}//${hostname}${port ? `:${port}` : ''}`
      console.log('[MobileCapture] ✓ Detected non-localhost domain, API URL:', baseUrl)
      return baseUrl
    }
  }
  
  // PRIORITY 4: Default to localhost for development
  console.log('[MobileCapture] Using default localhost:5000')
